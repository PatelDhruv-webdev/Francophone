  ...
  return (
    <div className="flex flex-col items-center">
      <div className="mb-4 flex w-full max-w-lg items-center justify-between">
        <button
          onClick={onExit}
          className="text-fg-muted hover:text-brand flex items-center gap-1 text-sm transition-colors"
        >
      ...
      >
        ...
        >
          ...
          >
            <p
              className="text-fg text-center text-3xl font-medium"
              style={{ fontFamily: 'var(--font-display)', letterSpacing: '0.02em' }}
            >
              {word.french}
            </p>
            {word.ipa && <p className="text-fg-subtle text-sm">{word.ipa}</p>}
            {word.gender && (
              <span
            ...
            <button
              ...
              className="text-fg-muted hover:bg-bg mt-2 rounded-full p-2 transition-colors"
              title="Listen"
            >
              <Volume2 className="h-4 w-4" />
            </button>
            <p className="text-fg-subtle mt-1 text-xs">Tap to reveal</p>
          </div>
          ...
          >
            <p className="text-fg text-center text-2xl font-medium">{word.english}</p>
            {word.example_fr && (
              <div className="mt-2 text-center">
                <p className="text-fg text-sm italic">{word.example_fr}</p>
                <p className="text-fg-subtle mt-0.5 text-xs">{word.example_en}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="mt-6 flex gap-4">
        <button
          onClick={prev}
          disabled={index === 0}
          className="text-fg hover:border-brand rounded-full border border-[rgba(30,27,22,0.1)] bg-white p-3 transition-colors disabled:opacity-30"
        >
          <ChevronLeft className="h-5 w-5" />
        </button>
        <button
          onClick={() => setFlipped((f) => !f)}
          className="text-fg hover:border-brand rounded-full border border-[rgba(30,27,22,0.1)] bg-white p-3 transition-colors"
          title="Flip"
        >
        ...
        <div className="mt-4 flex gap-1">
          {words.map((_, i) => (
            <button
              ...
              className={`h-1.5 w-1.5 rounded-full transition-colors ${i === index ? 'bg-brand' : 'bg-[#D4B896]'}`}
            />
          ))}
        </div>
      )}
  ...
  return (
    <div className="group rounded-xl border border-[rgba(30,27,22,0.08)] bg-white p-4 shadow-[0_2px_8px_rgba(30,27,22,0.06)] transition-shadow hover:shadow-[0_4px_12px_rgba(30,27,22,0.1)]">
      <div className="mb-1 flex items-start justify-between">
        <p
          className="text-fg text-lg font-medium tracking-wide"
          style={{ fontFamily: 'var(--font-display)', letterSpacing: '0.02em' }}
        >
          {word.french}
        </p>
        <button
          ...
          className="text-fg-subtle hover:text-brand rounded-full p-1.5 opacity-0 transition-colors group-hover:opacity-100 hover:bg-[#F5E8E3]"
          title="Listen"
        >
      ...
      {word.ipa && <p className="text-fg-subtle mb-1 text-xs">{word.ipa}</p>}

      <p className="text-fg-muted text-sm">{word.english}</p>

      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        <span className="bg-bg text-fg-muted rounded-full px-2 py-0.5 text-xs">
          {word.part_of_speech}
        </span>
        {word.gender && (
          <span
      ...
      {word.example_fr && (
        <div className="mt-2 border-t border-[rgba(30,27,22,0.05)] pt-2">
          <p className="text-fg-muted text-xs italic">{word.example_fr}</p>
          <p className="text-fg-subtle text-xs">{word.example_en}</p>
        </div>
      )}
    </div>
  )
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="text-fg placeholder-fg-subtle focus:border-brand focus:ring-brand/30 w-full rounded-lg border border-[rgba(30,27,22,0.15)] bg-white py-2 pr-4 pl-9 text-sm focus:ring-2 focus:outline-none"
          />
        </div>
        <button
          onClick={() => setFlashMode(true)}
          className="bg-brand hover:bg-brand-dark flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium whitespace-nowrap text-white transition-colors"
        >
